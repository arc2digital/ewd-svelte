<script>
  ...
  // Update the import to use ewdStore and its exports
  import { ewdStore, isConnected, subscribeToMessageType } from './stores/ewdStore.js';
  ...
  onMount(() => {
    ...
    ewdStore.connect({ // Changed from ewdStore
  ...
  async function sendMessageToServer() {
    ...
    try {
      ...
      const response = await ewdStore.sendMessage('hello/testHello', { text: messageText }); // Changed from ewdStore
      console.log('Response from server:', response);

      // ewdStore.sendMessage (using QEWD.reply) returns the full message object.
  ...
  function disconnectFromServer() {
    ewdStore.disconnect(); // Changed from ewdStore
